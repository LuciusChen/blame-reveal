...
All bindings use the =C-c l= prefix so the mode does not interfere with
normal text editing.

| Key         | Action                      |
|-------------+-----------------------------|
| =C-c l=     | Open transient menu         |
| =C-c l q=   | Toggle mode                 |
...
| =C-c l p=   | Go back in blame stack      |
| =C-c l g=   | Blame at specific revision  |
| =C-c l r=   | Reset to HEAD               |
| =C-c l f=   | Toggle focus mode           |
| =C-c l n=   | Next focus block            |
| =C-c l N=   | Previous focus block        |
...
Press =C-c l= in blame-reveal-mode to open the configuration menu:
