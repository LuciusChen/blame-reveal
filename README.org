...
blame-reveal uses a smart color system to highlight recent commits. See [[file:understanding-color-configuration.org][detailed color configuration guide]] for:

- How the three-step color pipeline works
...
;; Best clarity (very active files)
(setq blame-reveal-recent-days-limit 'auto
      blame-reveal-gradient-quality 'strict)

;; More history (exploration)
(setq blame-reveal-recent-days-limit 'auto
