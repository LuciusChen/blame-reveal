...
Navigate line history through parent commits (=C-c C-l b=), or lock onto a single commit to see all its modifications across the file (=C-c C-l f=).
...
(use-package blame-reveal
  ...
  ;; Optional for source checkouts: transient menu
...
Prefix keybindings (=C-c C-l=):

| Key         | Action                      |
|-------------+-----------------------------|
| =C-c C-l m= | Open transient menu         |
| =C-c C-l q= | Toggle mode                 |
| =C-c C-l c= | Copy commit hash            |
| =C-c C-l d= | Show commit diff            |
| =C-c C-l s= | Show commit details         |
| =C-c C-l h= | File history                |
| =C-c C-l l= | Line history                |
| =C-c C-l b= | Blame parent (recursive)    |
| =C-c C-l p= / =C-c C-l ^= | Go back in blame stack |
| =C-c C-l g= | Blame at specific revision  |
| =C-c C-l r= | Reset to HEAD               |
| =C-c C-l f= | Toggle focus mode           |
| =C-c C-l n= / =C-c C-l N= | Navigate focus blocks |
...
;; but most users should prefer presets from `C-c C-l m`.
#+end_src
...
- =C-c C-l m= → =Color Presets...=: choose =Heatmap=, =Green=, =Subtle=, =Vivid=
- =C-c C-l m= → =Recent Window...=: choose =Tight=, =Balanced=, =Wide=, =All=
...
Press =C-c C-l m= in =blame-reveal-mode= to open the configuration menu:
