...
(defvar blame-reveal-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "C-c l")   #'blame-reveal-menu)
    (define-key map (kbd "C-c l q") #'blame-reveal-mode)
    (define-key map (kbd "C-c l c") #'blame-reveal-copy-commit-hash)
    (define-key map (kbd "C-c l d") #'blame-reveal-show-commit-diff)
    (define-key map (kbd "C-c l s") #'blame-reveal-show-commit-details)
    (define-key map (kbd "C-c l h") #'blame-reveal-show-file-history)
    (define-key map (kbd "C-c l l") #'blame-reveal-show-line-history)
    (define-key map (kbd "C-c l b") #'blame-reveal-blame-recursively)
    (define-key map (kbd "C-c l p") #'blame-reveal-blame-back)
    (define-key map (kbd "C-c l g") #'blame-reveal-blame-at-revision)
    (define-key map (kbd "C-c l r") #'blame-reveal-reset-to-head)
    ;; Focus mode (requires blame-reveal-focus)
    (define-key map (kbd "C-c l f") #'blame-reveal-focus-commit)
    (define-key map (kbd "C-c l n") #'blame-reveal-next-focus-block)
    (define-key map (kbd "C-c l N") #'blame-reveal-prev-focus-block)
    map)
  "Keymap for blame-reveal-mode.
All bindings use the C-c l prefix to avoid interfering with text editing.")
